package dev.khanhtimn.gravitychanger.data.attachments;
...
public class GravityData {
    public static final Codec<GravityData> CODEC = RecordCodecBuilder.create(instance ->
            instance.group(
                    Direction.CODEC.fieldOf("baseGravityDirection").forGetter(d -> d.baseGravityDirection),
                    Codec.DOUBLE.fieldOf("baseGravityStrength").forGetter(d -> d.baseGravityStrength),
                    Direction.CODEC.fieldOf("currentGravityDirection").forGetter(d -> d.currGravityDirection),
                    Codec.DOUBLE.fieldOf("currentGravityStrength").forGetter(d -> d.currGravityStrength)
    ...
    public GravityData(Direction baseGravityDirection, double baseGravityStrength,
                       Direction currGravityDirection, double currGravityStrength) {
        ...
        this.prevGravityDirection = currGravityDirection;
        this.prevGravityStrength = currGravityStrength;
    }
        ...
        if (vehicle != null) {
            ...
            try {
                if (entity instanceof LivingEntity livingEntity) {
                    ...
                    // Iterate through active effects and apply them
                    for (MobEffectInstance effectInstance : livingEntity.getActiveEffects()) {
                        var effect = effectInstance.getEffect().value();
                        int amplifier = effectInstance.getAmplifier();

                        if (effect instanceof GravityDirectionMobEffect dirEffect) {
                            this.applyGravityDirectionEffect(
                                    dirEffect.gravityDirection,
                                    null,
                                    amplifier + 1.0
                            );
                        } else if (effect instanceof GravityStrengthMobEffect strengthEffect) {
                            int level = amplifier + 1;
                            this.applyGravityStrengthEffect(strengthEffect.getGravityStrengthMultiplier(level));
                        }
                    }

                    if (livingEntity.hasEffect(GravityMobEffects.INVERT)) {
                        this.applyGravityDirectionEffect(
    ...
    private void sendSyncPacketToOtherPlayers() {
        if (!this.entity.level().isClientSide()) {
            PacketDistributor.sendToPlayersTrackingEntityAndSelf(
                    this.entity,
                    new UpdateGravityDataPacket(
                            this.noAnimation,
                            this.entity.getUUID(),
                            baseGravityDirection,
                            currGravityDirection,
                            baseGravityStrength,
                            currGravityStrength
                    )
            );
    ...
    public void sync(boolean noAnimation, Direction baseGravityDirection, Direction currentGravityDirection,
        ...
        PacketDistributor.sendToServer(new UpdateGravitySyncStatePacket(this.entity.getUUID()));
    }
